...
void main()
{
	...
	else
	{
		...
		float attenuation_k1 = 0.2;
		float attenuation_k2 = 0.2;
		float attenuation_k3 = 0.2;
	...
	if (emitmode == 1) emissive = vec4(1.0, 1.0, 0.0, 1.0); 
	...
	if(useLighting == 1){
		outputColor = attenuation*texcolour*(fambientcolour + oren_nayar) + emissive + global_ambient;
	}else
	{
